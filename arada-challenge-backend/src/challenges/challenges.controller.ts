...
export class ChallengesController {
  ...
  @Get()
  getChallenges(
    @Query(ValidationPipe) filterDto: getChallengesFilterDto,
  ): Promise<Challenge[]> {
  ...
  @Patch(':id')
  update(
    @Body() updateChallengeDto: CreateChallengesDto,
    @Param('id') id,
  ): Promise<Challenge> {
  ...
  uploadData(
    @Body() challengeDTO: CreateChallengesDto,
    @UploadedFile('file') file: MulterFile,
  ) {
    if (!file) {
      throw new BadRequestException('File is not appropriate');
    } else {
      const filePathURL = `http://localhost:3000/challenges/viewImage/${file.filename}`;
      return this.challengesService.create(challengeDTO, filePathURL);
    }
  }
